...
class ciTerminalGUI:
    ...
    def __init__(self):
        ...
        citegresDefaultDbMenu.add_command(
            label="architecture", command=self.__citegresSetArchitectureDB
        )
        citegresDefaultDbMenu.add_command(
            label="social_computing", command=self.__citegresSetSocialComputingDB
        )
        citegresDefaultDbMenu.add_command(
            label="informatics", command=self.__citegresSetInformaticsDB
        )
        citegresDefaultDbMenu.add_command(
            label="engineering", command=self.__citegresSetEngineeringDB
        )
        ...
        advancedSearchButton = tk.Button(
            separatorSearchLabelFrame, text="XML SEARCH", command=self.__dblpQueryXML
        )
        ...
        self.__nordField = tk.Text(
            separatorNordStatusLabelFrame, state=tk.DISABLED, width=22, height=5
        )
        ...
        resultsFontDecreaseButton = tk.Button(
            ...
            text="Decrease Font",
            command=self.__decreaseResultsFieldFont,
        )
        resultsFontDecreaseButton.pack(side=tk.LEFT)
        resultsFontIncreaseButton = tk.Button(
            resultsFontButtonFrame,
            text="Increase Font",
            command=self.__increaseResultsFieldFont,
        )
        resultsFontIncreaseButton.pack(side=tk.RIGHT)

        ##### RESULTS TEXT FIELD
        self.__resultsFieldTextSize = 10
        self.__resultsField = tk.Text(
            separatorResultsTextLabelFrame,
            state=tk.DISABLED,
            yscrollcommand=lambda first, last: self.__resultsScrollBar.set(first, last),
        )
        self.__resultsField.pack(side=tk.BOTTOM, fill=tk.BOTH, expand=tk.YES)
        self.__resultsScrollBar = tk.Scrollbar(
            self.__resultsField, orient=tk.VERTICAL, cursor="arrow"
        )
        ...
        queryExtractExplodedXmlButton = tk.Button(
            ...
            text="Explode & Extract XML via Chrome",
            command=self.__dblpQueryExtract,
        )
        queryExtractExplodedXmlButton.pack(side=tk.TOP, fill=tk.X)
        queryImportXmlButton = tk.Button(
            separatorQueryDBLPLabelFrame,
            text="Import XML to Citegres",
            command=self.__citegresImportXML,
        )
        ...
        queryGetAuthorsAuthorsButton = tk.Button(
            ...
            command=self.__citegresGetAuthorsAuthors,
        )
        queryGetAuthorsAuthorsButton.pack(side=tk.TOP, fill=tk.X)
        queryGetSupportsAuthorsIdsButton = tk.Button(
            separatorQueryAuthorsLabelFrame,
            text="Get All Supporting Paper Author IDs",
            command=self.__citegresGetSupportsAuthorsIds,
        )
        queryGetSupportsAuthorsIdsButton.pack(side=tk.TOP, fill=tk.X)
        queryGetSupportsAuthorsResolvedButton = tk.Button(
            separatorQueryAuthorsLabelFrame,
            text="Resolve All Supporting Paper Authors",
            command=self.__citegresGetSupportsAuthorsResolved,
        )
        ...
        queryGetConceptsConceptsButton = tk.Button(
            ...
            text="Get All Concepts",
            command=self.__citegresConceptsConcepts,
        )
        queryGetConceptsConceptsButton.pack(side=tk.TOP, fill=tk.X)
        queryGetSupportsConceptsIdsButton = tk.Button(
            separatorQueryConceptsLabelFrame,
            text="Get All Paper Concept IDs",
            command=self.__citegresSupportsConceptsIds,
        )
        queryGetSupportsConceptsIdsButton.pack(side=tk.TOP, fill=tk.X)
        queryGetSupportsConceptsResolvedButton = tk.Button(
            separatorQueryConceptsLabelFrame,
            text="Resolve All Paper Concepts",
            command=self.__citegresSupportsConceptsResolved,
        )
        ...
        queryGetOpenalexIdEdgelistButton = tk.Button(
            ...
            text="Get Openalex ID Edgelist",
            command=self.__citegresGetOpenalexIdEdgelist,
        )
        queryGetOpenalexIdEdgelistButton.pack(side=tk.TOP, fill=tk.X)
        queryGetOpenalexUrlEdgelistButton = tk.Button(
            separatorQueryEdgelistLabelFrame,
            text="Get Openalex URL Edgelist",
            command=self.__citegresGetOpenalexUrlEdgelist,
        )
        queryGetOpenalexUrlEdgelistButton.pack(side=tk.TOP, fill=tk.X)
        queryGetOpenalexPaperEdgelistButton = tk.Button(
            separatorQueryEdgelistLabelFrame,
            text="Get Openalex Paper Edgelist",
            command=self.__citegresGetOpenalexPaperEdgelist,
        )
        queryGetOpenalexPaperEdgelistButton.pack(side=tk.TOP, fill=tk.X)
        queryGetOpenalexAuthorEdgelistButton = tk.Button(
            separatorQueryEdgelistLabelFrame,
            text="Get Openalex Author Edgelist",
            command=self.__citegresGetOpenalexAuthorEdgelist,
        )
        ...
        networkingAuthorCitationButton = tk.Button(
            ...
            text="Build Author Citation Graph",
            command=self.__networkingAuthorCitationGraph,
        )
        networkingAuthorCitationButton.pack(side=tk.TOP, fill=tk.X)
        networkingPaperCitationButton = tk.Button(
            separatorNetworkingLabelFrame,
            text="Build Paper Citation Graph",
            command=self.__networkingPaperCitationGraph,
        )
        networkingPaperCitationButton.pack(side=tk.TOP, fill=tk.X)
        networkingComputeGraphMetricsButton = tk.Button(
            separatorNetworkingLabelFrame,
            text="Compute Graph Metrics",
            command=self.__networkingComputeGraphMetrics,
        )
        networkingComputeGraphMetricsButton.pack(side=tk.TOP, fill=tk.X)
        networkingDisplayDegreeCentralities = tk.Button(
            separatorNetworkingLabelFrame,
            text="Display Degree Centralities",
            command=self.__networkingDisplayDegreeCentralities,
        )
        networkingDisplayDegreeCentralities.pack(side=tk.TOP, fill=tk.X)
        networkingDisplayClosenessCentralities = tk.Button(
            separatorNetworkingLabelFrame,
            text="Display Closeness Centralities",
            command=self.__networkingDisplayClosenessCentralities,
        )
        networkingDisplayClosenessCentralities.pack(side=tk.TOP, fill=tk.X)
        networkingDisplayBetweennessCentralities = tk.Button(
            separatorNetworkingLabelFrame,
            text="Display Betweenness Centralities",
            command=self.__networkingDisplayBetweennessCentralities,
        )
        networkingDisplayBetweennessCentralities.pack(side=tk.TOP, fill=tk.X)
        networkingPlotGraph = tk.Button(
            separatorNetworkingLabelFrame,
            text="Plot Graph",
            command=self.__networkingPlotGraph,
        )
        networkingPlotGraph.pack(side=tk.TOP, fill=tk.X)
        networkingGraphConfiguratorButton = tk.Button(
            separatorNetworkingLabelFrame,
            text="Deploy Graph Configurator",
            command=self.__networkingDeployGraphConfigurator,
        )
    ...
    def __citegresReconnect(self):
        self.__citegresStatus.set(1)
        self.__citegresCur, self.__citegresConn = postility.reset_connection(
    ...
    def __dblpQueryXML(self):
        query = self.__searchQuery.get()
        self.__searchResults = seleamility.chrome_query_dblp_XML(
            driver=self.__chromeDriver, query=query
        )
    ...
    def __dblpQueryExtract(self):
        ...
        self.__df = seleamility.explode_query_dblp(
            driver=self.__chromeDriver,
            search_results=self.__searchResults,
            use_nord=self.__nordStatus.get(),
        )
    ...
    def __increaseResultsFieldFont(self):
        ...
        self.__resultsField.config(
            font=font.Font(
                family="Courier New",
                size=self.__resultsFieldTextSize,
                weight="normal",
                slant="roman",
                underline=0,
                overstrike=0,
    ...
    def __citegresImportXML(self):
        self.__citegresCur, self.__citegresConn = postility.importXML(
    ...
    def __citegresGetSupportsAuthorsResolved(self):
        (self.__citegresCur, self.__citegresConn), status = (
            postility.select_all_from_supports_resolved(
                cur=self.__citegresCur, conn=self.__citegresConn
            )
        )
        self.__setResultsField(status.to_string())
    ...
    def __citegresSupportsConceptsIds(self):
        (self.__citegresCur, self.__citegresConn), status = (
            postility.select_all_from_paper_concepts(
                cur=self.__citegresCur, conn=self.__citegresConn
            )
        )
        self.__setResultsField(status.to_string())
    ...
    def __citegresSupportsConceptsResolved(self):
        (self.__citegresCur, self.__citegresConn), status = (
            postility.select_all_from_paper_concepts_resolved(
                cur=self.__citegresCur, conn=self.__citegresConn
            )
        )
        self.__setResultsField(status.to_string())
    ...
    def __citegresGetOpenalexUrlEdgelist(self):
        (self.__citegresCur, self.__citegresConn), status = (
            postility.query_citationlist_full_resolve_openalex(
                cur=self.__citegresCur, conn=self.__citegresConn
            )
        )
        self.__setResultsField(status.to_string())
    ...
    def __citegresGetOpenalexPaperEdgelist(self):
        (self.__citegresCur, self.__citegresConn), status = (
            postility.query_citationlist_full_resolve_paper_title(
                cur=self.__citegresCur, conn=self.__citegresConn
            )
        )
        self.__setResultsField(status.to_string())
    ...
    def __citegresGetOpenalexAuthorEdgelist(self):
        (self.__citegresCur, self.__citegresConn), status = (
            postility.query_citationlist_full_resolve_author(
                cur=self.__citegresCur, conn=self.__citegresConn
            )
        )
        self.__setResultsField(status.to_string())
    ...
    def __networkingAuthorCitationGraph(self):
        (self.__citegresCur, self.__citegresConn), self.__df = (
            postility.query_citationlist_full_resolve_author(
                cur=self.__citegresCur, conn=self.__citegresConn
            )
        )
    ...
    def __networkingDisplayDegreeCentralities(self):
        results = pd.DataFrame(
            {
                self.__gType: [node for node in self.__G.nodes()],
                "degree_centralities": self.__metrics["betweenness_centralities"],
            }
        ).sort_values("degree_centralities", ascending=False)
    ...
    def __networkingDisplayClosenessCentralities(self):
        results = pd.DataFrame(
            {
                self.__gType: [node for node in self.__G.nodes()],
                "closeness_centralities": self.__metrics["betweenness_centralities"],
            }
        ).sort_values("closeness_centralities", ascending=False)
        self.__setResultsField(results.to_string())
    ...
    def __networkingDisplayBetweennessCentralities(self):
        results = pd.DataFrame(
            {
                self.__gType: [node for node in self.__G.nodes()],
                "betweenness_centrality": self.__metrics["betweenness_centralities"],
            }
    ...
    def __networkingPlotGraph(self):
        ...
        netility.plot_graph(
            G=self.__G,
            pos=self.__pos,
            use_labels=self.__use_labels,
            alpha=self.__alpha,
            edge_color=self.__edge_color,
            width=self.__edge_width,
            arrowsize=self.__arrow_size,
    ...
    def __networkingDeployGraphConfigurator(self):
        # INIT
        self.__guiConfigurator = tk.Tk(
            screenName="graphConfigurator", className="graphConfigurator"
        )
        ...
        configurationIncreaseAlphaButton = tk.Button(
            ...
            text="Increase Alpha",
            command=self.__configurationIncreaseAlpha,
        )
        configurationIncreaseAlphaButton.pack(side=tk.TOP, fill=tk.X)
        configurationDecreaseAlphaButton = tk.Button(
            separatorGraphConfigurator,
            text="Decrease Alpha",
            command=self.__configurationDecreaseAlpha,
        )
